...
A `postBuild` or `start` file in the repository can be used to insure the running container does any last steps needed or retrieves data from remote sites to make your repository lightweight and more flexible. Which one you use depends on what you want 'baked' into the Docker image associated with the repository, see [here](https://mybinder.readthedocs.io/en/latest/using.html#share-computational-work-or-papers). A limitation that novices may encounter is that making use of `postBuild` necesitates the use of git because the file has to be executable, and I have found editing it directly in Github's browser interface seems to break this ability in a way that is not obvious as the file will still be tagged as executable in Github's browser interface. 
